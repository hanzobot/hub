...
const DEFAULT_SITE_URL = 'https://skills.com'
...
function buildDescription(event: WebhookEvent, skill: WebhookSkillPayload) {
  ...
  if (event === 'skill.highlighted') return 'Newly highlighted skill on Skills.'
  if (skill.version) return `New version v${skill.version} published on Skills.`
  return 'New skill published on Skills.'
}

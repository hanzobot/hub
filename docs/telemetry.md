---
summary: 'Install telemetry collected via `skills sync` + opt-out.'
read_when:
...
Skills uses **minimal telemetry** to compute **install counts** (what’s actually in use) and to power better sorting/filtering.
This is based on the CLI `skills sync` command.
...
On each `skills sync`, the CLI reports a **full snapshot** of what it found, grouped by scan root (“folder/root”).
...
Skills provides a private “Installed” tab on your own profile:
...
With this set, the CLI will not send telemetry during `skills sync`.

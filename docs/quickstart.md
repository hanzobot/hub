...
Install a skill into `./skills/<slug>` (if Bot is configured, installs into that workspace instead):

```bash
bun skills install <slug>
bun skills list
```
...
```bash
bun skills install <slug> --workdir /tmp/skills-demo --dir skills
```

Update:

```bash
bun skills update --all
```
...
```bash
mkdir -p /tmp/skills-skill-demo && cd /tmp/skills-skill-demo
cat > SKILL.md <<'EOF'
---
name: Demo Skill
...
```bash
bun skills sync --all --dry-run --no-input
```

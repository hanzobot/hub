...
- CLI: auto-scan Bot skill roots (per-agent workspaces, shared skills, extraDirs).
...
- CLI: default workdir falls back to Bot workspace (override with `--workdir` / `SKILLS_WORKDIR`).
...
### Added
- Telemetry: track installs via `skills sync` (logged-in only), per root, with 120-day staleness.
...
### Changed
- CLI: telemetry opt-out via `SKILLS_DISABLE_TELEMETRY=1`.
...
- CLI: ignore legacy `auth.skills.com` registry and prefer site discovery.
...
- CLI package: bundle schema internally (no external `skills-schema` publish).
- Repo: mark `skills-schema` as private to prevent publishing.
